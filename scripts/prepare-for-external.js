/**
 * Prepare For External Deployment Script
 * 
...
const VITE_CONFIG_BACKUP = path.join(ROOT_DIR, 'vite.config.replit.ts');
...
const START_SCRIPT_LINUX_CONTENT = `#!/bin/bash
...
const START_SCRIPT_WINDOWS_CONTENT = `@echo off
...
async function backupViteConfig() {
  if (fs.existsSync(VITE_CONFIG_PATH)) {
    fs.copyFileSync(VITE_CONFIG_PATH, VITE_CONFIG_BACKUP);
    console.log(`Backed up original vite.config.ts to vite.config.replit.ts`);
  }
}
...
async function createNewViteConfig() {
  try {
    ...
    console.log(`Created new vite.config.ts without Replit dependencies`);
  } catch (error) {
    console.error(`Error creating new vite config: ${error.message}`);
  }
}
...
async function createStartScripts() {
  try {
    ...
    fs.writeFileSync(START_SCRIPT_LINUX, START_SCRIPT_LINUX_CONTENT);
    fs.chmodSync(START_SCRIPT_LINUX, '755'); // Make executable
    console.log(`Created start.sh script for Linux/Mac`);
    
    // Create Windows script
    fs.writeFileSync(START_SCRIPT_WINDOWS, START_SCRIPT_WINDOWS_CONTENT);
    console.log(`Created start.bat script for Windows`);
  } catch (error) {
...
// Main function
async function main() {
  console.log('Preparing project for external deployment...');
  
  // Step 1: Backup existing vite config
  await backupViteConfig();
  
  // Step 2: Create new vite config
  ...
  // Step 6: Create platform-specific start scripts
  await createStartScripts();
  
  console.log('\nPreparation complete! Your project is now ready to run outside of Replit.');
  ...
  console.log('\nMake sure to check the .env file and update any required values!');
}
